...
///
/// # TODO
/// - Consider switching the return type to `alloy_primitives::B256` for consistency with other
...
#[derive(Debug)]
pub enum Error {
    /// Triggered when an operation reaches an unresolved node. The associated
    /// `alloy_primitives::B256` value provides details about the unresolved node.
            ...
            rlp::Prototype::List(17) => {
                ...
                for node_rlp in rlp.iter().take(16) {
                    match node_rlp.prototype()? {
                        ...
                        _ => node_list
                            .push(Some(alloc::boxed::Box::new(rlp::Decodable::decode(&node_rlp)?))),
                    }
                }
...
impl From<MptNodeData> for MptNode {
    fn from(value: MptNodeData) -> Self {
        Self { data: value, cached_reference: core::cell::RefCell::new(None) }
    }
}
                ...
                if remaining.next().is_none() {
                    ...
                    match &mut orphan.data {
                        ...
                        MptNodeData::Leaf(prefix, orphan_value) => {
                            let new_nibs: alloc::vec::Vec<_> =
                                core::iter::once(index as u8).chain(prefix_nibs(prefix)).collect();
...
pub fn resolve_state_nodes(
    ...
    let trie = match root.as_data() {
        ...
        MptNodeData::Leaf(_key, _value) => {
            ...
            let account =
                <alloy_trie::TrieAccount as alloy_rlp::Decodable>::decode(&mut &_value[..])
                    .unwrap();
