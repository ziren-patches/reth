...
impl ZkvmTrie {
    pub fn from_execution_witness(
        ...
        let (state_trie, storage_tries) =
            Self::build_validated_tries(witness, pre_state_root).unwrap();
    ...
    > {
        ...
        let root =
            root_node.unwrap_or_else(|| crate::mpt::MptNodeData::Digest(pre_state_root).into());
        ...
        for (hashed_address, storage_root) in storage_tries_detected {
            let root_node = match node_by_hash.get(&storage_root).cloned() {
                Some(node) => node,
                None => {
                    // An execution witness can include an account leaf (with non-empty
                    // storageRoot), but omit its entire storage trie when that
                    // account's storage was NOT touched during the block.
                    continue;
                }
            };
        ...
        // Step 3a: Verify that state_trie was built correctly - confirm tree hash with pre state
        // root.
        validate_state_trie(&state_trie, pre_state_root);

        // Step 3b: Verify that each storage trie matches the declared storage_root in the state
        // trie.
    ...
    pub fn account(
        ...
        Option<alloy_trie::TrieAccount>,
        reth_ethereum::evm::primitives::execute::ProviderError,
    > {
        let hashed_address = alloy_primitives::keccak256(address);
        let hashed_address = hashed_address.as_slice();

        let account_in_trie =
            self.state_trie.get_rlp::<alloy_trie::TrieAccount>(hashed_address).unwrap();
...
impl reth_stateless::StatelessTrie for ZkvmTrie {
    fn new(
